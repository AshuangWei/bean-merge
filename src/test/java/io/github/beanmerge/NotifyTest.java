...
import io.github.beanmerge.base.data.ObjectWithCustomFieldA;
import io.github.beanmerge.base.data.ObjectWithCustomFieldB;
import io.github.beanmerge.base.data.SimpleObjectA;
import io.github.beanmerge.base.data.SimpleObjectB;
import io.github.beanmerge.base.data.SubObject;
...
public class NotifyTest {
  ...
  @Test
  public void should_success_notify_when_any_path_changed() {
    ObjectWithCustomFieldA from = Dummie.withStrategy(GenerationStrategy.RANDOM)
        .create(ObjectWithCustomFieldA.class);
    ObjectWithCustomFieldB to = Dummie.create(ObjectWithCustomFieldB.class);
    from.getCustomFieldA().setScalarTypeDouble(to.getCustomFieldA().getScalarTypeDouble());
    ...
    Merge.withConfiguration(new MergeConfiguration()
        .notifyUpdateAny(Arrays.asList("customFieldA.scalarTypeFloat", "customFieldA.scalarTypeDouble"),
            (name, f, t) -> mock.setSubObjectIntegerField(1)))
        .merge(from, to);
  ...
  @Test
  public void should_not_notify_when_none_path_changed_with_any_mode() {
    ...
    from.getCustomFieldA().setScalarTypeFloat(to.getCustomFieldA().getScalarTypeFloat());
    from.getCustomFieldA().setScalarTypeDouble(to.getCustomFieldA().getScalarTypeDouble());
    SubObject mock = mock(SubObject.class);
